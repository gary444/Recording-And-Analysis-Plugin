...
extern "C" int UNITY_INTERFACE_EXPORT UNITY_INTERFACE_API ProcessAnalysisRequests(int analysis_id, float* intervals, std::vector<TimeInterval> intervals_of_interest) {
    AnalysisManager& manager = AnalysisManager::getInstance();
...
extern "C" int UNITY_INTERFACE_EXPORT UNITY_INTERFACE_APIGetObjectPositions(int analysis_id, int object_id, float start_time, float end_time, float* positions, int max_positions) {
    AnalysisManager& manager = AnalysisManager::getInstance();
    return manager.get_object_positions(object_id, start_time, end_time, positions, max_positions);
}

extern "C" void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_APIClearAnalysisRequests(int analysis_id) {
    AnalysisManager& manager = AnalysisManager::getInstance();
    return manager.clear_requests();
}

extern "C" void UNITY_INTERFACE_EXPORT UNITY_INTERFACE_APIClearAnalysisFilePaths() {
    AnalysisManager& manager = AnalysisManager::getInstance();
